...
def main():
    ...
    for j in range(3):  # описываем правила с тремя попытками
        ...
        missing_number_index = random.randint(0, len(progression) - 1)  # случайная позиция для недостающего числа
# создаем вопрос с двумя точками вместо числа
        question = " ".join(str(x) if i != missing_number_index else ".." for i, x in enumerate(progression))
...
# проверяем совпадает ли ответ пользователя с правильным ответом
        if user_answer == vacancy:
            ...
            print(f"'{user_answer}' is wrong answer ;(. Correct answer was '{vacancy}'.")
